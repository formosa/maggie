import argparse,os,queue,threading,time
...
class EventBus:
	def __init__(self):self.subscribers={};self.queue=queue.PriorityQueue();self.running=False;self._worker_thread=None
...
class MaggieAI:
	def __init__(self,config:Dict[str,Any]):self.config=config;self.state=State.IDLE;self.event_bus=EventBus();self.extensions={};self.inactivity_timer=None;self.inactivity_timeout=self.config.get('inactivity_timeout',300);self.hardware_manager=None;self.wake_word_detector=None;self.stt_processor=None;self.llm_processor=None;self.gui=None;self.thread_pool=ThreadPoolExecutor(max_workers=self.config.get('cpu',{}).get('max_threads',10),thread_name_prefix='maggie_thread_');self.transition_handlers={State.IDLE:self._on_enter_idle,State.READY:self._on_enter_ready,State.ACTIVE:self._on_enter_active,State.CLEANUP:self._on_enter_cleanup,State.SHUTDOWN:self._on_enter_shutdown};self._register_event_handlers();self._setup_gpu_resource_management()
	def _handle_state_transition(self,new_state:State,trigger:str)->None:
		...
		old_state=self.state;self.state=new_state;logger.info(f"State transition: {old_state.name} -> {new_state.name} (trigger: {trigger})");from maggie.utils.hardware.gpu_utils import clear_gpu_cache
		if new_state in[State.IDLE,State.CLEANUP,State.SHUTDOWN]:clear_gpu_cache()
	...
	def _setup_gpu_resource_management(self)->None:
		try:
			...
			if torch.cuda.is_available():
				logger.info(f"GPU Resource Management: Setup for the {torch.cuda.get_device_name(0)}");torch.cuda.empty_cache();logger.info('PyTorch CUDA caching allocator enabled')
				if self.config.get('logging',{}).get('console_level','INFO')=='DEBUG':torch.autograd.set_detect_anomaly(True);logger.info('PyTorch anomaly detection enabled')
				else:torch.autograd.set_detect_anomaly(False);logger.info('PyTorch anomaly detection disabled')
				if hasattr(torch.cuda,'memory_reserved'):total_memory=torch.cuda.get_device_properties(0).total_memory;logger.info(f"Total GPU memory == {total_memory/1024**3:.2f}GB");memory_reserved=torch.cuda.memory_reserved(0);logger.info(f"memory_reserved == {memory_reserved/1024**3:.2f}GB")
		except ImportError:logger.debug('PyTorch not available for GPU resource management')
		except Exception as e:logger.error(f"Error setting up GPU resource management: {e}")
	...
	def start(self)->bool:
		try:
			...
			self._transition_to(State.IDLE,'startup');self.hardware_manager.start_monitoring();logger.info('Maggie AI Assistant started successfully');return True
		except Exception as e:logger.error(f"Error starting Maggie AI: {e}");return False
	def stop(self)->bool:
		try:logger.info('Stopping Maggie AI Assistant');self._transition_to(State.SHUTDOWN,'stop_requested');logger.info('Maggie AI Assistant stopped successfully');return True
		except Exception as e:logger.error(f"Error stopping Maggie AI: {e}");return False
	...
	def _on_enter_idle(self,transition:StateTransition)->None:
		...
		try:
			import torch
			if torch.cuda.is_available():torch.cuda.empty_cache();logger.debug('GPU memory cache cleared in IDLE state')
	...
	def _on_enter_cleanup(self,transition:StateTransition)->None:
		...
		try:
			import torch
			if torch.cuda.is_available():torch.cuda.empty_cache();logger.debug('GPU memory cache cleared in CLEANUP state')
		except ImportError:pass
	...
	def _on_enter_shutdown(self,transition:StateTransition)->None:
		if self.hardware_manager:self.hardware_manager.stop_monitoring()
		self.event_bus.stop()
		try:
			import torch
			if torch.cuda.is_available():torch.cuda.empty_cache();logger.debug('GPU memory freed during shutdown')
		except ImportError:pass
		self.thread_pool.shutdown(wait=True,timeout=5);logger.info('Entered SHUTDOWN state - application will exit')
	...
	def shutdown(self)->bool:
		...
		if hasattr(self,'gui')and self.gui:pass
		return True
	def timeout(self)->None:
		if self.state==State.READY or self.state==State.ACTIVE:logger.info('Manual sleep requested');self.tts_processor.speak('Going to sleep');self._transition_to(State.CLEANUP,'manual_timeout')
