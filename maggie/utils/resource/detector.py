"""
Maggie AI Assistant - Hardware Detection Module
=============================================

Hardware detection functionality for the Maggie AI Assistant.

This module provides comprehensive hardware detection capabilities,
identifying CPU, memory, GPU, and disk specifications with particular
optimization for AMD Ryzen 9 5900X and NVIDIA RTX 3080 hardware.
"""
...
class HardwareDetector:
    """
    Hardware detection for system components.
    
    This class provides methods to detect and identify hardware components
    including CPU, memory, GPU, and disk, with special detection for
    AMD Ryzen 9 5900X and NVIDIA RTX 3080 hardware.
    
    Methods
    -------
    detect_system()
        Detect all system components and return consolidated information
    detect_cpu()
        Detect CPU model, cores, and capabilities
    detect_memory()
        Detect memory size, type, and speed
    detect_gpu()
        Detect GPU model, memory, and CUDA capabilities
    detect_disk()
        Detect disk space and type
    """
    ...
    def detect_system(self) -> Dict[str, Any]:
        ...
        -------
        Dict[str, Any]
            Dictionary with system hardware information
        """
        system_info = {
            'os': {
        ...
        if system_info['cpu']['is_ryzen_9_5900x']:
            ...
            logger.info(f"Detected CPU: {system_info['cpu']['model']} with "
                       f"{system_info['cpu']['physical_cores']} cores")
        ...
        if system_info['gpu']['is_rtx_3080']:
            ...
            logger.info(f"Detected GPU: {system_info['gpu']['name']} with "
                       f"{system_info['gpu']['memory_gb']:.2f}GB VRAM")
        else:
    ...
    def detect_memory(self) -> Dict[str, Any]:
        """
        Detect memory size, type and specifications.
        ...
        try:
            ...
            # Windows-specific detection for memory type/speed
            if platform.system() == 'Windows':
                try:
                    import wmi
                    c = wmi.WMI()
    ...
    def detect_gpu(self) -> Dict[str, Any]:
        """
        Detect GPU model and specifications.
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                if '3080' in gpu_info['name']:
                    ...
                    # Windows-specific detection for more GPU details
                    if platform.system() == 'Windows':
                        try:
                            import pynvml
                            pynvml.nvmlInit()
                            ...
                            logger.debug('PYNVML module not available for detailed GPU detection')
    ...
    def detect_disk(self) -> Dict[str, Any]:
        ...
        # Windows-specific detection for SSD
        if platform.system() == 'Windows':
            try:
                import wmi
                c = wmi.WMI()
