...
class HardwareManager:
    ...
    def _check_resource_thresholds(self,cpu_percent:float,memory:psutil._psplatform.svmem,gpu_util:Optional[Dict[str,float]])->None:
        ...
        if gpu_util:
            ...
            elif gpu_util['memory_percent']>90:
                logger.warning(f"High GPU memory usage: {gpu_util['memory_percent']:.1f}% - "+
                              f"allocated: {gpu_util['memory_allocated']/1024**3:.1f} GB")
    ...
    # Methods integrated from gpu_utils.py
    def clear_gpu_cache(self) -> bool:
        """
        Clears the GPU memory cache to free up VRAM.
    ...
    def get_gpu_info(self) -> Dict[str, Any]:
        ...
        """
        # This functionality is already largely covered by _detect_gpu()
        return self.hardware_info['gpu']
    
    def optimize_for_rtx_3080(self) -> Dict[str, Any]:
        """
        Apply optimizations specifically for RTX 3080 GPU.
        ...
        try:
            ...
            if torch.cuda.is_available() and '3080' in torch.cuda.get_device_name(0):
                ...
                # Enable memory stats if available
                if hasattr(torch.cuda, 'memory_stats'):
                    optimizations['settings']['memory_stats_enabled'] = True
                
                # Set cuDNN benchmark mode for optimized convolutions
                ...
                # Set maximum split size for memory allocation
            ...
            logger.debug('PyTorch not available for GPU optimizations')
            return optimizations
        except Exception as e:
            logger.error(f"Error applying GPU optimizations: {e}")
            return optimizations
    
    def test_gpu_compatibility(self) -> Tuple[bool, List[str]]:
        """
        Test if the GPU is compatible with the application requirements.
        
        Returns
        -------
        Tuple[bool, List[str]]
            Success status and list of warnings
        """
        warnings = []
        ...
        try:
            ...
            if gpu_info['memory_gb'] < 8:
                warnings.append(f"GPU memory ({gpu_info['memory_gb']:.1f}GB) is less than recommended 8GB")
            ...
            if '3080' in gpu_info['name']:
                ...
                if not gpu_info.get('cuda_version', '').startswith('11.'):
                    warnings.append(f"CUDA version {gpu_info['cuda_version']} - version 11.x recommended for RTX 3080")
            
            return True, warnings
        except ImportError:
            logger.debug('PyTorch not available for GPU compatibility testing')
            return False, ['Error testing GPU compatibility: PyTorch not available']
        except Exception as e:
            logger.error(f"Error testing GPU compatibility: {e}")
            return False, [f'Error testing GPU compatibility: {e}']
    ...
    def _apply_cpu_optimizations(self,cpu_info:Dict[str,Any])->None:
        if cpu_info.get('is_ryzen_9_5900x',False):
            ...
            self.config.setdefault('cpu',{})
            self.config['cpu']['max_threads']=8
            self.config['cpu']['thread_timeout']=30
    ...
    def _apply_memory_optimizations(self,memory_info:Dict[str,Any])->None:
        if memory_info.get('is_32gb',False):
            ...
            self.config['memory']['max_percent']=75
            self.config['memory']['model_unload_threshold']=85
            self.config.setdefault('tts',{})
            self.config['tts']['cache_size']=200
            self.config.setdefault('stt',{}).setdefault('whisper_streaming',{})
            self.config['stt']['whisper_streaming']['buffer_size_seconds']=3e1
    ...
    def _apply_gpu_optimizations(self,gpu_info:Dict[str,Any])->None:
        if gpu_info.get('is_rtx_3080',False):
            ...
            self.config.setdefault('stt',{})
            self.config['stt'].setdefault('whisper',{})
            self.config['stt']['whisper']['compute_type']='float16'
            self.config['stt'].setdefault('whisper_streaming',{})
            ...
            self.config['gpu']['max_percent']=90
            self.config['gpu']['model_unload_threshold']=95