...
class MainWindow(QMainWindow):
    ...
    def _on_input_state_change(self,requested_state:str)->None:
        # Fixed state transition calls using state_manager
        if self.maggie_ai.state == State.IDLE and requested_state == 'ACTIVE':
            self.maggie_ai._transition_to(State.READY, 'input_field_activated')
    ...
    def _on_input_submitted(self,text:str)->None:
        ...
        # Fixed state check to use the state property
        if self.maggie_ai.state == State.IDLE:
            self.maggie_ai._transition_to(State.READY, 'user_input')
