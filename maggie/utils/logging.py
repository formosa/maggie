...
class LoggingManager:
	...
	def __init__(self,config:Dict[str,Any]):self.config=config.get('logging',{});self.log_dir=Path(self.config.get('path','logs')).resolve();self.log_dir.mkdir(exist_ok=True,parents=True);self.console_level=self.config.get('console_level','INFO');self.file_level=self.config.get('file_level','DEBUG');self._enhanced_logging=False;self._event_publisher=None;self._error_handler=None;self._state_provider=None;self.log_batch_size=self.config.get('batch_size',50);self.log_batch_timeout=self.config.get('batch_timeout',5.);self.async_logging=self.config.get('async_enabled',True);self.correlation_id=None;self._configure_basic_logging()
	...
	def enhance_with_event_publisher(self,event_publisher)->None:self._event_publisher=event_publisher;self._enhanced_logging=True
	def enhance_with_error_handler(self,error_handler)->None:self._error_handler=error_handler;self._enhanced_logging=True
	def enhance_with_state_provider(self,state_provider)->None:self._state_provider=state_provider;self._enhanced_logging=True
	def log(self,level:LogLevel,message:str,*args,exception:Optional[Exception]=None,**kwargs)->None:
		if level==LogLevel.DEBUG:logger.debug(message,*args,exc_info=exception,**kwargs)
		elif level==LogLevel.INFO:logger.info(message,*args,exc_info=exception,**kwargs)
		elif level==LogLevel.WARNING:logger.warning(message,*args,exc_info=exception,**kwargs)
		elif level==LogLevel.ERROR:logger.error(message,*args,exc_info=exception,**kwargs)
		elif level==LogLevel.CRITICAL:logger.critical(message,*args,exc_info=exception,**kwargs)
...
class ComponentLogger:
	def __init__(self,component_name:str):self.component=component_name;self.logger=logging.getLogger(component_name)
