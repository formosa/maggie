...
class LoggingManager:
    ...
    def _get_hardware_detector(self):
        ...
        if self._hardware_detector is None:
            try:
                ...
                logger.warning(
                    "Failed to import HardwareDetector, system info may be limited")
                self._hardware_detector = None
    ...
    def _initialize_async_logging(self) -> None:
        ...
        self.log_worker = threading.Thread(
            target=self._process_log_queue, name='AsyncLogWorker', daemon=True)
    ...
    def _flush_log_batch(self) -> None:
        ...
        with self.log_batch_lock:
            ...
            self.log_batch_timer = threading.Timer(
                self.log_batch_timeout, self._flush_log_batch)
            self.log_batch_timer.daemon = True
    ...
    def _log_system_info(self) -> None:
        ...
        try:
            ...
            if hardware_detector:
                ...
                logger.info(
                    f"System: {system_info['os']['system']} {system_info['os']['release']}")
                ...
                logger.info(
                    f"RAM: {memory_info['total_gb']:.2f} GB (Available: {memory_info['available_gb']:.2f} GB)")
                gpu_info = system_info['gpu']
                if gpu_info.get('available', False):
                    logger.info(
                        f"GPU: {gpu_info['name']} ({gpu_info['memory_gb']:.2f} GB VRAM)")
                    logger.info(f"CUDA available: {gpu_info['cuda_version']}")
                    if gpu_info.get('is_rtx_3080', False):
                        logger.info(
                            'RTX 3080 detected - applying optimal configurations')
                else:
                    logger.warning(
                        'CUDA not available, GPU acceleration disabled')
            else:
                logger.warning(
                    "Hardware detector not available, skipping detailed system info")
    ...
    def log(self, level: LogLevel, message: str, *args, **kwargs) -> None:
        ...
        """
        if self.async_logging and self.log_queue is not None:
            self.log_queue.put((level, message, args, kwargs))
        ...
        if LogDestination.EVENT_BUS not in self.enabled_destinations:
            ...
            def handle_log(record):
                if record['level'].name in ('ERROR', 'CRITICAL'):
                    data = {'message': record['message'], 'level': record['level'].name, 'time': record['time'].isoformat(
                    ), 'name': record['name'], 'function': record['function'], 'line': record['line'], 'module': record['module'], 'correlation_id': record.get('extra', {}).get('correlation_id')}
                    try:
                        from maggie.service.locator import ServiceLocator
                        state_manager = ServiceLocator.get('state_manager')
                        if state_manager:
                            current_state = state_manager.get_current_state()
                            data['state'] = current_state.name
                            data['state_style'] = current_state.get_style()
                    except Exception:
                        pass
                    event_bus.publish('error_logged', data)
            logger.configure(
                handlers=[{'sink': handle_log, 'level': 'ERROR'}], extra=logger._core.extra)
    ...
    def setup_global_exception_handler(self) -> None:
        ...
        def global_exception_handler(exc_type, exc_value, exc_traceback):
            if issubclass(exc_type, KeyboardInterrupt):
                ...
                return
            logger.opt(exception=(exc_type, exc_value, exc_traceback)
            ...
            try:
                ...
                if event_bus:
                    ...
                    if state_manager:
                        current_state = state_manager.get_current_state()
                        state_info = {
                            'current_state': current_state.name, 'style': current_state.get_style()}
                    error_data = {'type': str(exc_type.__name__), 'message': str(exc_value), 'traceback': ''.join(traceback.format_tb(
                        exc_traceback)), 'is_unhandled': True, 'correlation_id': self.correlation_id, 'state_info': state_info}
    ...
    def log_performance(self, component: str, operation: str, elapsed: float, details: Dict[str, Any] = None) -> None:
        ...
        perf_logger = logger.bind(
            performance=True, component=component, operation=operation)
        perf_logger.debug(log_entry)
        if elapsed > 1.:
    ...
    def log_state_transition(self, from_state: Any, to_state: Any, trigger: str) -> None:
        ...
        fsm_logger.info(
            f"State transition: {from_name} -> {to_name} (trigger: {trigger})")
        logger.info(
            f"State transition: {from_name} -> {to_name} (trigger: {trigger})")
    ...
    def log_resource_allocation(self, resource_type: str, resource_name: str, state: Any, details: Dict[str, Any] = None) -> None:
        ...
        log_msg = f"Allocated {resource_type}/{resource_name} in state {state_name}"
        if details:
            detail_str = ', '.join(f"{k}={v}"for (k, v) in details.items())
            log_msg += f" ({detail_str})"
        resource_logger = logger.bind(
            resource_operation=True, component='ResourceManager', resource_type=resource_type)
        resource_logger.info(log_msg)
    ...
    def log_resource_deallocation(self, resource_type: str, resource_name: str, state: Any, details: Dict[str, Any] = None) -> None:
        ...
        """
        state_name = state.name if hasattr(state, 'name')else str(state)
        log_msg = f"Deallocated {resource_type}/{resource_name} in state {state_name}"
        if details:
            detail_str = ', '.join(f"{k}={v}"for (k, v) in details.items())
            log_msg += f" ({detail_str})"
        resource_logger = logger.bind(
    ...
    def log_input_processing(self, input_type: str, state: Any, details: Dict[str, Any] = None) -> None:
        ...
        """
        state_name = state.name if hasattr(state, 'name')else str(state)
        log_msg = f"Processing {input_type} in state {state_name}"
        if details:
            detail_str = ', '.join(f"{k}={v}"for (k, v) in details.items())
            log_msg += f" ({detail_str})"
        input_logger = logger.bind(
            input_operation=True, component='InputProcessor', input_type=input_type)
        input_logger.info(log_msg)
    ...
    def shutdown(self) -> None:
        ...
        if self.log_batch_enabled:
            self._flush_log_batch()
            if self.log_batch_timer:
                self.log_batch_timer.cancel()
        if self.async_logging and self.log_queue:
            self.log_queue.put(None)
            if self.log_worker:
                self.log_worker.join(timeout=2.)
...
def logging_context(correlation_id: Optional[str] = None, component: str = '', operation: str = '', state: Any = None) -> Generator[Dict[str, Any], None, None]:
    ...
    context = {'correlation_id': ctx_id, 'component': component,
    ...
    if state is None:
        try:
            ...
            if state_manager:
                current_state = state_manager.get_current_state()
                context['state'] = current_state.name if hasattr(
                    current_state, 'name')else str(current_state)
        except Exception:
            pass
    try:
        logging_mgr = LoggingManager.get_instance()
        prev_id = logging_mgr.get_correlation_id()
        logging_mgr.set_correlation_id(ctx_id)
    except RuntimeError:
        prev_id = None
        ...
        try:
            ...
            try:
                if prev_id is not None:
                    logging_mgr = LoggingManager.get_instance()
                    logging_mgr.set_correlation_id(prev_id)
            except RuntimeError:
                pass
            if component and operation:
                elapsed = time.time()-context['start_time']
                logger.debug(
                    f"{component}/{operation} completed in {elapsed:.3f}s")
                if elapsed > .1:
                    try:
                        logging_mgr = LoggingManager.get_instance()
                        logging_mgr.log_performance(
                            component, operation, elapsed)
                    except RuntimeError:
                        pass
        ...
        def wrapper(*args, **kwargs):
            ...
            with logging_context(component=component, operation=operation, state=state)as ctx:
                if log_args and args_str:
                    logger.debug(f"{operation} called with args: {args_str}")
                start_time = time.time()
                result = func(*args, **kwargs)
                elapsed = time.time()-start_time
                if log_result:
                    if isinstance(result, (str, int, float, bool, type(None))):
                        logger.debug(f"{operation} returned: {result}")
                    else:
                        logger.debug(
...
class ComponentLogger:
    ...
    def _get_state_context(self) -> Dict[str, Any]:
        ...
        if self._state_manager:
            try:
                ...
                context['state'] = current_state.name
            except Exception:
                pass
        return context
        ...
        if self._resource_manager:
            try:
                ...
                if 'memory' in resource_status:
                    context['memory_percent'] = resource_status['memory'].get(
                        'used_percent', 0)
                if 'gpu' in resource_status and resource_status['gpu'].get('available', False):
                    context['gpu_percent'] = resource_status['gpu'].get(
                        'memory_percent', 0)
            except Exception:
                pass
        return context
    ...
    def error(self, message: str, exception: Optional[Exception] = None, **kwargs) -> None:
        ...
        if exception:
            self.logger.bind(
                **context).error(f"{message}: {exception}", exc_info=exception, **kwargs)
        else:
    ...
    def critical(self, message: str, exception: Optional[Exception] = None, **kwargs) -> None:
        ...
        if exception:
            self.logger.bind(
                **context).critical(f"{message}: {exception}", exc_info=exception, **kwargs)
        else:
            self.logger.bind(**context).critical(message, **kwargs)
    ...
    def log_state_change(self, old_state: Any, new_state: Any, trigger: str) -> None:
        ...
        """
        old_name = old_state.name if hasattr(
            old_state, 'name')else str(old_state)
        new_name = new_state.name if hasattr(
            new_state, 'name')else str(new_state)
        context = {'fsm': True, 'from_state': old_name,
                   'to_state': new_name, 'trigger': trigger}
        self.logger.bind(
            **context).info(f"State change: {old_name} -> {new_name} (trigger: {trigger})")
        try:
            logging_mgr = LoggingManager.get_instance()
    ...
    def log_performance(self, operation: str, elapsed: float, details: Dict[str, Any] = None) -> None:
        ...
        try:
            logging_mgr = LoggingManager.get_instance()
            logging_mgr.log_performance(
                self.component, operation, elapsed, details)
        except RuntimeError:
            pass
