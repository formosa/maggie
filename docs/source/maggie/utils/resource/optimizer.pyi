...
class HardwareOptimizer:
    def __init__(self, hardware_info: Dict[str, Any], config: Dict[str, Any]):
        ...
        self._state_specific_profiles = {}
        self._transition_profiles = {}
        
        try:
            self.state_manager = ServiceLocator.get('state_manager')
            if self.state_manager:
                self._initialize_state_profiles()
        except Exception:
            pass
            
    def _initialize_state_profiles(self) -> None:
        from maggie.core.state import State
        
        self._state_specific_profiles = {
            State.INIT: {
                'cpu': {'priority': 'normal', 'affinity': 'minimal'},
                'gpu': {'enabled': False},
                'memory': {'preload': False, 'clear_cache': True}
            },
            State.STARTUP: {
                'cpu': {'priority': 'above_normal', 'affinity': 'balanced'},
                'gpu': {'enabled': True, 'memory_percent': 30},
                'memory': {'preload': False, 'clear_cache': True}
            },
            State.IDLE: {
                'cpu': {'priority': 'below_normal', 'affinity': 'efficient'},
                'gpu': {'enabled': False},
                'memory': {'preload': False, 'clear_cache': True}
            },
            State.LOADING: {
                'cpu': {'priority': 'high', 'affinity': 'performance'},
                'gpu': {'enabled': True, 'memory_percent': 90, 'tensor_cores': True},
                'memory': {'preload': True, 'clear_cache': False}
            },
            State.READY: {
                'cpu': {'priority': 'normal', 'affinity': 'balanced'},
                'gpu': {'enabled': True, 'memory_percent': 60},
                'memory': {'preload': True, 'clear_cache': False}
            },
            State.ACTIVE: {
                'cpu': {'priority': 'high', 'affinity': 'performance'},
                'gpu': {'enabled': True, 'memory_percent': 80, 'tensor_cores': True},
                'memory': {'preload': True, 'clear_cache': False}
            },
            State.BUSY: {
                'cpu': {'priority': 'high', 'affinity': 'performance'},
                'gpu': {'enabled': True, 'memory_percent': 95, 'tensor_cores': True},
                'memory': {'preload': True, 'clear_cache': False}
            },
            State.CLEANUP: {
                'cpu': {'priority': 'normal', 'affinity': 'balanced'},
        ...
        from maggie.core.state import StateTransition
    ...
    def create_optimization_profile(self) -> Dict[str, Any]:
        profile = {
            ...
            'input_processing': self._optimize_input_processing()
        }
    ...
    def get_state_specific_profile(self, state=None) -> Dict[str, Any]:
        ...
        if state in self._state_specific_profiles:
            return self._state_specific_profiles[state]
            
        return {}
        
    def get_transition_profile(self, from_state, to_state) -> Dict[str, Any]:
        return self._transition_profiles.get((from_state, to_state), {})
    ...
    def _optimize_threading(self) -> Dict[str, Any]:
        ...
        if cpu_info.get('is_ryzen_9_5900x', False):
            return {
                'max_workers': 10,
                'thread_timeout': 30,
                'worker_affinity': list(range(10)),
                'priority_boost': True,
                'use_dedicated_threads': True,
                'numa_aware': True,
                'power_plan': 'High performance',
                'core_parking': False,
                'smt_enabled': True,
                'background_thread_priority': 'below_normal',
                'thread_scheduler': 'dynamic',
                'performance_cores': [0, 1, 2, 3, 4, 5, 6, 7],
                'background_cores': [8, 9, 10, 11],
                'io_threads': [8, 9],
                'compute_threads': [0, 1, 2, 3, 4, 5, 6, 7],
                'thread_affinity_strategy': 'performance_first',
                'thread_affinity_enabled': True,
                'thread_priority_scheme': 'adaptive'
            }
        else:
    ...
    def _optimize_gpu(self) -> Dict[str, Any]:
        ...
        if gpu_info.get('is_rtx_3080', False):
            return {
                ...
                'stream_priority': {
                    'high': ['inference', 'training'],
                    'normal': ['dataloading'],
                    'low': ['garbage_collection']
                },
                'texture_cache': True,
                'asynchronous_execution': True,
                'cuda_malloc_async': True,
                'fused_attention': True,
                'memory_defrag_interval': 100,
                'precision_combo': 'mixed_float16',
    ...
    def _optimize_audio(self) -> Dict[str, Any]:
        ...
        if gpu_info.get('is_rtx_3080', False):
            audio_opt.update({
                ...
                'cache_models': True,
                'vad_sensitivity': .6,
                'cuda_audio_processing': True,
                'parallel_processing': True,
                'precision': 'mixed',
                'use_tensor_cores': True,
                'audio_cache_size_mb': 512,
                'tts_batch_size': 64,
                'stt_batch_size': 16,
                'spectral_processing': 'gpu'
            })
    ...
    def _optimize_input_processing(self) -> Dict[str, Any]:
        ...
        if self.hardware_info['gpu'].get('is_rtx_3080', False):
            input_opt.update({
                ...
                'parallel_processing': True,
                'streaming_mode': True,
                'gpu_accelerated_vad': True,
                'confidence_threshold': .6,
                'enhance_partial_results': True,
                'gpu_accelerated_nlp': True
            })
            
        if self.hardware_info['cpu'].get('is_ryzen_9_5900x', False):
            input_opt.update({
                'dedicated_audio_thread': True,
                'wake_word_sensitivity': .7,
                'input_buffering': 'adaptive',
                'voice_activity_min_duration_ms': 300
            })
    ...
    def optimize_for_current_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            profile = self.get_state_specific_profile(current_state)
            
            if 'cpu' in profile:
                self._apply_cpu_optimizations(profile['cpu'])
                
            if 'gpu' in profile:
                self._apply_gpu_optimizations(profile['gpu'])
                
            if 'memory' in profile:
                self._apply_memory_optimizations(profile['memory'])
    ...
    def _apply_cpu_optimizations(self, cpu_profile: Dict[str, Any]) -> None:
        try:
            if platform.system() == 'Windows':
                ...
                priority = cpu_profile.get('priority', 'normal')
                priority_map = {
                ...
                if affinity_type == 'performance' and self.hardware_info['cpu'].get('is_ryzen_9_5900x', False):
                    process.cpu_affinity(list(range(10)))
                elif affinity_type == 'minimal':
    ...
    def _apply_gpu_optimizations(self, gpu_profile: Dict[str, Any]) -> None:
        try:
            if not gpu_profile.get('enabled', True):
                return
    ...
    def optimize_for_busy_state(self) -> Dict[str, Any]:
        # Adding this method since it's referenced in manager.py
        # Implementing a basic version that just calls optimize_for_current_state
        return self.optimize_for_current_state()