...
class ProgressTracker:
    def __init__(self, color: ColorOutput, total_steps: int = 8):
        self.color = color
...
class MaggieInstaller:
    ...
    def _download_file(self, url: str, destination: str, show_progress: bool = True, retries: int = 3) -> bool:
        for attempt in range(1, retries + 1):
            try:
                self.color.print(f"Downloading {url} (Attempt {attempt}/{retries})", 'blue')
                ...
                if attempt < retries:
                    self.color.print("Retrying...", 'yellow')
    ...
    def _detect_gpu(self) -> Dict[str, Any]:
        ...
        if self.cpu_only:
            return gpu_info
        try:
            returncode, stdout, _ = self._run_command(['nvcc', '--version'], check=False)
            if returncode == 0:
                for line in stdout.splitlines():
                    if 'release' in line:
                        gpu_info['cuda_version'] = line.split('release')[1].strip().split(',')[0]
                        gpu_info['cuda_available'] = True
            python_cmd = self._get_venv_python() if os.path.exists(self._get_venv_python()) else sys.executable
            returncode, stdout, _ = self._run_command([python_cmd, '-c', "import torch; print(f'CUDA: {torch.cuda.is_available()}'); print(f'Device: {torch.cuda.get_device_name(0)}') if torch.cuda.is_available() else None; print(f'CUDA Version: {torch.version.cuda}') if torch.cuda.is_available() else None"], check=False)
            if returncode != 0:
                if self.verbose:
            ...
            if gpu_info['cuda_available']:
                returncode, stdout, _ = self._run_command([python_cmd, '-c', "import torch; print(f'VRAM: {torch.cuda.get_device_properties(0).total_memory / (1024**3):.2f}')"], check=False)
    ...
    def _setup_virtual_env(self) -> bool:
        ...
        if os.path.exists(venv_dir):
            if self.force_reinstall:
                self.color.print('Removing existing virtual environment due to --force-reinstall', 'yellow')
    ...
    def _install_basic_dependencies(self, python_cmd: str) -> bool:
        ...
        cmd = [python_cmd, '-m', 'pip', 'install'] + (['--force-reinstall'] if self.force_reinstall else []) + basic_deps
    ...
    def _install_pytorch(self, python_cmd: str) -> bool:
        cuda_version = self._detect_cuda_version() if not self.cpu_only else None
        if self.cpu_only or not cuda_version:
            self.color.print('Installing PyTorch (CPU version)...', 'cyan')
            cmd = [python_cmd, '-m', 'pip', 'install', 'torch==2.0.1', 'torchvision==0.15.2', 'torchaudio==2.0.2'] + (['--force-reinstall'] if self.force_reinstall else [])
        else:
            self.color.print(f'Installing PyTorch with CUDA {cuda_version} support...', 'cyan')
            cuda_ver = cuda_version.replace(".", "")
            cmd = [python_cmd, '-m', 'pip', 'install', f'torch==2.0.1+cu{cuda_ver}', f'torchvision==0.15.2+cu{cuda_ver}', f'torchaudio==2.0.2+cu{cuda_ver}', '--extra-index-url', f'https://download.pytorch.org/whl/cu{cuda_ver}'] + (['--force-reinstall'] if self.force_reinstall else [])
        returncode, _, stderr = self._run_command(cmd)
        if returncode != 0:
            ...
            if returncode == 0:
                for line in stdout.splitlines():
                    if 'release' in line:
                        ...
                        return version
            return "11.8"  # Default to CUDA 11.8 if detection fails but GPU is present
        except:
            return None
    ...
    def _install_dependencies(self) -> bool:
        ...
        try:
            if os.path.exists(req_path):
                ...
                cmd = [python_cmd, '-m', 'pip', 'install', '-r', temp_req_path] + (['--force-reinstall'] if self.force_reinstall else [])
                returncode, _, stderr = self._run_command(cmd)
                ...
                self.color.print('No requirements.txt found, skipping standard dependencies', 'yellow')
        except Exception as e:
    ...
    def _install_specialized_dependencies(self, python_cmd: str) -> bool:
        ...
        if not self.cpu_only:
            ...
            self._run_command([python_cmd, '-m', 'pip', 'install', 'onnxruntime-gpu==1.15.1'] + (['--force-reinstall'] if self.force_reinstall else []))
        return True
    ...
    def _install_pyaudio(self, python_cmd: str) -> bool:
        ...
        if returncode == 0 and not self.force_reinstall:
            self.color.print('PyAudio already installed', 'green')
            return True
        if self.platform_system == 'Windows':
            wheel_url = 'https://files.pythonhosted.org/packages/27/bc/719d140ee63cf4b0725016531d36743a797ffdbab85e8536922902c9349a/PyAudio-0.2.14-cp310-cp310-win_amd64.whl'
            ...
            if not self._download_file(wheel_url, wheel_path):
                ...
                return False
            returncode, _, stderr = self._run_command([python_cmd, '-m', 'pip', 'install', wheel_path] + (['--force-reinstall'] if self.force_reinstall else []))
            if returncode == 0:
                ...
                returncode, _, stderr = self._run_command([python_cmd, '-m', 'pip', 'install', 'PyAudio==0.2.14'] + (['--force-reinstall'] if self.force_reinstall else []))
                if returncode == 0:
            ...
            if returncode == 0:
                ...
                self.color.print('Then try: pip install PyAudio==0.2.14', 'yellow')
                return False
    ...
    def _install_kokoro(self, python_cmd: str) -> bool:
        ...
        if returncode == 0 and not self.force_reinstall:
            ...
            return True
        self._run_command([python_cmd, '-m', 'pip', 'install', 'numpy', 'tqdm', 'soundfile'] + (['--force-reinstall'] if self.force_reinstall else []))
        if not self.cpu_only and self.hardware_info['gpu']['cuda_available']:
            self._run_command([python_cmd, '-m', 'pip', 'install', 'onnxruntime-gpu==1.15.1'] + (['--force-reinstall'] if self.force_reinstall else []))
        else:
            self._run_command([python_cmd, '-m', 'pip', 'install', 'onnxruntime==1.15.1'] + (['--force-reinstall'] if self.force_reinstall else []))
        if self.has_git:
            returncode, _, stderr = self._run_command([python_cmd, '-m', 'pip', 'install', 'git+https://github.com/hexgrad/kokoro.git'] + (['--force-reinstall'] if self.force_reinstall else []))
            if returncode == 0:
        ...
        try:
            ...
            returncode, _, stderr = self._run_command([python_cmd, '-m', 'pip', 'install', kokoro_dir] + (['--force-reinstall'] if self.force_reinstall else []))
            if returncode == 0:
    ...
    def _install_whisper(self, python_cmd: str) -> bool:
        ...
        if returncode == 0 and not self.force_reinstall:
            ...
            return True
        returncode, _, stderr = self._run_command([python_cmd, '-m', 'pip', 'install', 'faster-whisper==0.9.0', 'soundfile==0.12.1'] + (['--force-reinstall'] if self.force_reinstall else []))
        if returncode != 0:
    ...
    def _download_whisper_model(self) -> bool:
        ...
        if os.path.exists(model_dir):
            ...
            if all(essential_file in files_in_dir for essential_file in essential_files) and not self.force_reinstall:
                self.color.print('Whisper base.en model is already available', 'green')
                return True
            else:
                self.color.print('Whisper model directory exists but appears incomplete or --force-reinstall is set', 'yellow')
                self.color.print('Will attempt to download/update the model', 'yellow')
        os.makedirs(model_dir, exist_ok=True)
        try:
            python_cmd = self._get_venv_python()
            self._run_command([python_cmd, '-m', 'pip', 'install', 'huggingface_hub'] + (['--force-reinstall'] if self.force_reinstall else []))
            check_script = '''
import os
...
'''
    ...
    def _download_af_heart_model(self) -> bool:
        ...
        MIN_SIZE = 41 * 1024 * 1024  # Minimum expected size in bytes
        if os.path.exists(model_path):
            file_size = os.path.getsize(model_path)
            if file_size >= MIN_SIZE and not self.force_reinstall:
                self.color.print(f"af_heart voice model verified ({file_size / (1024 * 1024):.2f} MB)", 'green')
                return True
            else:
                self.color.print(f"af_heart model has incorrect size: {file_size / (1024 * 1024):.2f} MB or --force-reinstall is set", 'yellow')
    ...
    def _download_mistral_model(self) -> bool:
        ...
        if os.path.exists(mistral_dir) and os.listdir(mistral_dir):
            ...
            if not missing_files and not failed_patterns and not self.force_reinstall:
                self.color.print('Mistral model is available and appears complete', 'green')
                return True
            else:
                self.color.print('Mistral model directory exists but appears incomplete or --force-reinstall is set', 'yellow')
                if missing_files:
        ...
        if self.has_git:
            ...
            if os.path.exists(mistral_dir) and os.listdir(mistral_dir):
                try:
                    ...
                    self.color.print('Cleared existing Mistral model directory', 'yellow')
                except Exception as e:
    ...
    def _create_recipe_template(self) -> bool:
        ...
        try:
            python_cmd = self._get_venv_python()
            returncode, _, stderr = self._run_command([python_cmd, '-m', 'pip', 'install', 'python-docx>=0.8.11'] + (['--force-reinstall'] if self.force_reinstall else []))
            if returncode != 0:
                self.color.print(f"Error installing python-docx: {stderr}", 'red')
                return False
            returncode, _, stderr = self._run_command([python_cmd, '-c', f'''
import docx
...
'''])
            if returncode == 0:
                ...
                self.color.print(f"Error creating recipe template: {stderr}", 'red')
                return False
        except Exception as e:
    ...
    def _setup_config(self) -> bool:
        ...
        if not os.path.exists(example_path) and os.path.exists(alt_example_path):
            try:
                ...
                return False
        if not os.path.exists(example_path) and not os.path.exists(config_path):
            self.color.print('No configuration example file found, creating default config', 'yellow')
            default_config = {
                'stt': {'whisper': {'model_path': 'models/stt/whisper-base.en'}},
                'tts': {'voice_model': 'af_heart.pt'},
                'llm': {'model_path': 'maggie/models/llm/mistral-7b-instruct-v0.3-GPTQ-4bit'}
            }
            try:
                import yaml
                with open(config_path, 'w') as f:
                    yaml.dump(default_config, f, default_flow_style=False)
                self.color.print('Default configuration file created', 'green')
            except Exception as e:
                self.color.print(f"Error creating default config: {e}", 'red')
                return False
        import json
